...
export default function FrontendWidget() {
		...
		try {
			...
			if (!cid) {
				const conv = await apiFetch({
					...
					method: 'POST',
					data:   { title: text.slice(0, 60), post_id: currentPostId },
				});
				cid = conv.id;
				setConvId(cid);
			}
	...
	return (
		<div className="wpaim-widget">
			<div className={`wpaim-widget__panel ${open ? '' : 'wpaim-widget__panel--hidden'}`}>
				...
				<div className="wpaim-widget__messages">
					{messages.length === 0 && (
						<p style={{ color: 'var(--color-text-muted)', fontSize: 'var(--text-sm)', textAlign: 'center', marginTop: 'var(--space-6)' }}>
				...
				<div className="wpaim-widget__composer">
					...
					>
						{isLoading ? <Loader2 size={14} className="wpaim-spin" /> : <Send size={14} />}
					</button>
				</div>
			</div>
